...
View() {
    ...
    # 自动检测节点名称
    local node_name=$(auto_node_name)
    ...
    if [[ "${ipv4}" != "IPv4_Error" ]]; then
        ss_url_ipv4="ss://${userinfo}@${ipv4}:${config_port}#${node_name}"
    fi
    if [[ "${ipv6}" != "IPv6_Error" ]]; then
        ss_url_ipv6="ss://${userinfo}@${ipv6}:${config_port}#${node_name}"
    fi
    ...
    if [[ "${ipv4}" != "IPv4_Error" ]]; then
        echo -e "${node_name} = ss, ${ipv4}, ${config_port}, encrypt-method=${config_method}, password=${config_password}, tfo=${config_tfo}, udp-relay=true"
    fi
    if [[ "${ipv6}" != "IPv6_Error" ]]; then
        echo -e "${node_name} = ss, ${ipv6}, ${config_port}, encrypt-method=${config_method}, password=${config_password}, tfo=${config_tfo}, udp-relay=true"
    fi
    ...
    if [ -f "/etc/systemd/system/shadowtls-ss.service" ]; then
        ...
        local ss_stls_url="ss://${userinfo}@${ipv4}:${config_port}?shadow-tls=${shadow_tls_base64}#${node_name}"
        ...
        if [[ "${ipv4}" != "IPv4_Error" ]]; then
            echo -e "${node_name} = ss, ${ipv4}, ${stls_listen_port}, encrypt-method=${config_method}, password=${config_password}, shadow-tls-password=${stls_password}, shadow-tls-sni=${stls_sni}, shadow-tls-version=3, udp-relay=true"
        fi
        if [[ "${ipv6}" != "IPv6_Error" ]]; then
            echo -e "${node_name} = ss, ${ipv6}, ${stls_listen_port}, encrypt-method=${config_method}, password=${config_password}, shadow-tls-password=${stls_password}, shadow-tls-sni=${stls_sni}, shadow-tls-version=3, udp-relay=true"
        fi
    fi
...
auto_node_name() {
    ...
    # 使用 Cloudflare API 获取 ISP 信息
    # $26 通常是城市/地区信息，$18 通常是 ISP 信息
